...
date: "2025-10-07"
categories: ["ebpf"]
tags: ["ebpf", "linux", "verifier"]
slug: "bpf-state-pruning"
---

The version of the kernel code for these notes is `v6.17`.

The main verifier loop lies in the `do_check()` function, going over each
instruction. State pruning happens early in that function, checking if the
instruction is a prune point and then checking if the state is visited with the
following code:
...
So what qualifies an instruction to be a prune point?

## Prune points

Prune point fields are written and accessed through helpers:
...
The above code from `do_check()` is the only place `is_prune_point` is called.
For marking prune points, this is done during the parsing of the instructions.

### Parsing the eBPF program

The entry point of the BPF verifier is the `bpf_check` function, called from
the `bpf_prog_load` function in `syscall.c`, it runs the eBPF verifier. From
...
single "edge", that's why it's calling `push_insn` with `t + off + 1`. We can
safely ignore everything that's between the jump and where it jumps since it's
unconditional, the code won't be executed.

The landing point of the jump is marked as a pruning point (4/8).
...
instruction (5/8) and then call `push_insn` two times, on time with `t + 1`
...
times above. First we visit continue to visit the code after the call by
calling `push_insn` with `t + insn_sz` with edge `FALLTHROUGH`, this is
marked as a prune point (7/8). Then, if we visit the callee, we `push_insn` at
the callee code level and put another prune point at the current function call
instruction note that it's not `w` but `t` here (8/8).
...
2) the wide instruction loading an address, which will be another type of BPF
to BPF call[^1]. When we visit the callee, we call `push_insn` now with `w` and
thus place a pruning point at the landing point.
...
Let's try to picture all possibilities of pruning points in BPF bytecode.

#### Pseudo func load

First we need to take care the `bpf_pseudo_func` instruction, meaning a
`LDIMM64` with `src_reg` being `BPF_PSEUDO_FUNC 4`. Prunes points are marked
...
[...]
12:  xx xx xx xx xx xx xx xx <instruction>
13:  xx xx xx xx xx xx xx xx <instruction> [PRUNE_POINT]
```
...
##### Open coded iterator next function
...
```
 0:  85 00 00 00 12 00 00 00 call +0x12 [PRUNE_POINT]
 1:  xx xx xx xx xx xx xx xx <instruction> [PRUNE_POINT]
[...]
12:  xx xx xx xx xx xx xx xx <instruction>
13:  xx xx xx xx xx xx xx xx <instruction> [PRUNE_POINT]
