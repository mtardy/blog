---
title: "First contact: try Tetragon on Linux"
date: "2023-04-27"
categories: ["ebpf"]
tags: ["security", "ebpf", "open-source", "linux"]
slug: "try-tetragon-on-linux"
description: "Discover and experiment with Tetragon on your local Linux host!"
canonicalURL: "https://tetragon.io/docs"
ShowCanonicalLink: true
cover:
    image: "https://raw.githubusercontent.com/cilium/tetragon/main/docs/assets/icons/logo.svg"
    alt: "Logo of the Tetragon project"
---

> This guide was originally published in the [Tetragon documentation](https://tetragon.io/docs).
> It was first part of the getting started guides, but was later replaced and
> move to the tutorials section, that was later removed. Since we don't want to
> maintain this guide anymore, it would have a good end of life on a blog if it
> can be useful to some people.

---

Note that this guide is not a tutorial on how to deploy Tetragon standalone
(i.e. without Kubernetes), you can see the
[container deployment](https://tetragon.io/docs/installation/container) and
[package deployment](https://tetragon.io/docs/installation/package) guides for
...
`5.15.0` and `5.19.0` on amd64 and arm64 but [any recent
distribution](https://github.com/libbpf/libbpf#bpf-co-re-compile-once--run-everywhere)
shipping with a relatively recent kernel should work. See the FAQ for further
details on the [recommended kernel
versions](https://tetragon.io/docs/faq#what-is-the-minimum-linux-kernel-version-to-run-tetragon).

Note that you cannot run Tetragon using Docker Desktop on macOS because of a
limitation of the Docker Desktop Linux virtual machine. Learn more about this
issue and how to run Tetragon on a Mac computer in [this section of the FAQ
page](https://tetragon.io/docs/faq/#can-i-run-tetragon-on-mac-computers).
...
[development setup](https://tetragon.io/docs/contribution-guide/development-setup) to see how to
build the binaries.
...
perform [process lifecycle observability](https://tetragon.io/docs/use-cases/process-lifecycle).
...
Note that, since Tetragon monitors all processes on the host, you may observe
events unrelated to what was typed in the shell session. Indeed, Tetragon is
...
[TracingPolicy documentation](https://tetragon.io/docs/concepts/tracing-policy).
...
Note that, the syscall we choose is `openat` and not `open` because the glibc
and most of the implementations of the standard C library use the `openat`
syscall for their `open` wrapper. So `cat` on Linux is using `openat`.
...
The `openat` syscall can take relative paths as arguments and even absolute path
filtering can often be bypassed by pseudo filesystems like procfs, for example
using a prefix like `/proc/self/root/` to access `/`.
...
to modify the `action` in the `matchActions` filter from `Override` and add
`Sigkill`.
